...
const generateBookingDetails = (booking: Booking, currency: string, rate: number) => {
  let nights = Math.ceil((booking.checkOut.getTime() - booking.checkIn.getTime()) / (1000 * 60 * 60 * 24));
  let pricePerNight = Math.round(booking.totalAmount / nights);
  let taxes = Math.round(booking.totalAmount * 0.12);
  let subtotal = booking.totalAmount - taxes;
  
  const TAX_RATE = 0.12;

  if (currency === "inr") {
    const totalInr = booking.inr_amount;       // fixed gross amount
    subtotal = Math.round(totalInr / (1 + TAX_RATE)); 
    taxes = totalInr - subtotal;               // remainder is tax
    pricePerNight = Math.round(totalInr / nights);
  }
...
export function BookingDetailsModal({ booking, isOpen, onClose, onCancel }: BookingDetailsModalProps) {
  ...
  const {currency, rate, currencyConverter, symbol} = useCurrency()
  const details = generateBookingDetails(booking, currency, rate);
            ...
            <CardContent className="p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm">{symbol}{details.subtotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm">Taxes & Fees</span>
                  <span className="text-sm">{symbol}{details.taxes.toLocaleString()}</span>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <span className="text-base">Total Amount</span>
                  <span className="text-lg text-green-600">{symbol}{ currency == "usd" ? booking.totalAmount.toLocaleString() : booking.inr_amount}</span>
                </div>
